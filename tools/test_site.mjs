...
await test('Unicode coordinates and eight complete print sheets',()=>{assert.equal(chartCode(0xf2a00,3,11),0xf2ab3);const sheets=printableSheets(catalogue.blocks);assert.equal(sheets.length,8);const codes=sheets.flatMap(sheet=>Array.from({length:128},(_,index)=>sheet.start+index));assert.equal(new Set(codes).size,1024);assert.equal(codes[0],0xf2a00);assert.equal(codes.at(-1),0xf2dff);});
...
const pages=['index.html','charts.html','specimens.html','proposal.html','downloads.html'];
await test('Five progressive pages share the controls and semantic navigation',async()=>{for(const page of pages){const html=await read(`dist/${page}`);assert(html.startsWith('<!doctype html>'));for(const id of ['main','font-weight','font-weight-value','font-italic','font-status'])assert.equal((html.match(new RegExp(`id="${id}"`,'g'))||[]).length,1,`${page}: ${id}`);assert(html.includes('aria-current="page"'));assert(html.includes('<noscript>'));assert(!/conlang|vowels|consonants|readingPairs|proof-data\.json/i.test(html),page);}});
await test('Four screen grids, eight print grids, and 832 numeric names',async()=>{const html=await read('dist/charts.html');assert.equal((html.match(/data-chart-kind="screen"/g)||[]).length,4);assert.equal((html.match(/data-chart-kind="print"/g)||[]).length,8);const codes=[...html.matchAll(/data-name-codepoint="([A-F\d]+)"/g)].map(match=>parseInt(match[1],16));assert.equal(codes.length,832);assert.deepEqual(codes,[...codes].sort((a,b)=>a-b));assert.equal(new Set(codes).size,832);assert.equal((html.match(/class="vacant"/g)||[]).length,384);for(const entry of catalogue.entries)assert(html.includes(`id="u-${entry.codePoint.toString(16)}"`));});
